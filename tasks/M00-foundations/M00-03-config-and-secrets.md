...
- [ ] Defaults load with no files present; `config.path` reports the paths.
- [ ] User and workspace layers merge with correct precedence; `sources`
      reports the winning layer per key; env override wins over files.
- [ ] Workspace file with a non-overridable key fails with a clear error
      naming the key and file.
- [ ] `set` round-trips through `toml_edit` preserving comments.
- [ ] Secret set/get/delete works via keychain on Windows (Credential
      Manager) and via `FileStore`; `ANTHROPIC_API_KEY` env takes precedence.
- [ ] `Secret` never appears in `Debug`/logs (test greps a captured log).
